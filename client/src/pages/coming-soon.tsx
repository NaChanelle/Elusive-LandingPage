...
import { Clock, Mail, Menu, ChevronUp, Users, ChevronLeft, ChevronRight, Quote, Shield, Lightbulb, Crown, X } from "lucide-react";
          ...
          <Accordion type="single" collapsible className="space-y-4">
            {(content as any)?.faq_items?.map((item: any, index: number) => (
              <AccordionItem key={`item-${index}`} value={`item-${index}`} className="border border-gray-700 rounded-lg px-6">
                <AccordionTrigger className="text-white hover:text-[#FFB90F] text-left">
                  {item.question}
                </AccordionTrigger>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex flex-col items-center space-y-4">
            ...
            <div className="flex justify-center space-x-8 text-sm text-gray-400">
              <a href="mailto:hello@elusive.quest" className="hover:text-[#FFB90F] transition-colors">Contact</a>
              <button onClick={() => setPrivacyOpen(true)} className="hover:text-[#FFB90F] transition-colors">Privacy</button>
              <button onClick={() => setTermsOpen(true)} className="hover:text-[#FFB90F] transition-colors">Terms</button>
            </div>
          </div>
        </div>
      </footer>
